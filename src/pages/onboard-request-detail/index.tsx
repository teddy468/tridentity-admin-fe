...
const OnboardDetailPage = () => {
  ...
  const getDetail = async () => {
    try {
      const res = await userService.fetchOboardingRequestDetail(id);
      if (res.data && res.status === STATUS_CODE.SUCCESS) {
        ...
        const arrayAccountInformation = [
          ...
          },
          {
            title: "Category",
            value:
              res.data.categories?.map((item: any) => item.name).length > 0 &&
              _.join(
                res.data.categories?.map((item: any) => item.name),
                `, `
              ),
            isReject: false,
          },
  ...
  const confirm = (
    ...
    setOpenModalReviewInformation(false);
    Modal.confirm({
      title: "Confirm",
      ...
      onOk: () => {
        if (textAction === "decline") {
          handleDeclineMerchant(merchantId, rejectedFields!);
        } else {
          handleAcceptMerchant(merchantId);
        }
      },
    });
  };
  ...
  const handleAcceptMerchant = async (merchantId: number) => {
    try {
      setLoading(true);
  ...
  return (
    <>
      <TridentityPageHeader title="Onboard request detail">
      ...
      <ModalDetailMerchantInformationForApproval
        ...
        handleActionMerchant={confirm}
      />
    </>
  );
};

export default OnboardDetailPage;
